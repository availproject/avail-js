...
async function tx_test() {
...

  {
    ...
    const submittable = client
      .tx()
      .staking()
      .payoutStakersByPage("0x37dfeeed435f0e9f205e1dfc55775fcd06518f63a5b1ccd53ce2d9e14ab783d3", 417, 0)
...

  {
    ...
    const submittable = client
      .tx()
      .staking()
      .payoutStakers("0xa4605eebf32be28f4b30219a329d5f61d1b250c2780ca62f1875e84adeac8b42", 422)
...
async function storage_test() {
  ...
  eq(isOkNotNull(await staking.storage.CounterForNominators.fetch(client.endpoint, blockHash)), 2920)
  eq(isOkNotNull(await staking.storage.CounterForValidators.fetch(client.endpoint, blockHash)), 136)
  eq(isOkNotNull(await staking.storage.CurrentEra.fetch(client.endpoint, blockHash)), 420)
  eq(isOkNotNull(await staking.storage.CurrentPlannedSession.fetch(client.endpoint, blockHash)), 2522)
  eq(isOk(await staking.storage.MaxNominatorsCount.fetch(client.endpoint, blockHash)), null)
  eq(isOk(await staking.storage.MaxValidatorsCount.fetch(client.endpoint, blockHash)), null)
  eq(json(isOkNotNull(await staking.storage.MinNominatorBond.fetch(client.endpoint, blockHash))), json(ONE_K_AVAIL))
  eq(json(isOkNotNull(await staking.storage.MinValidatorBond.fetch(client.endpoint, blockHash))), json(FIFTY_K_AVAIL))
  eq(
    json(isOkNotNull(await staking.storage.MinimumActiveStake.fetch(client.endpoint, blockHash))),
    json(new BN("1743992038550")),
  )
  eq(isOkNotNull(await staking.storage.MinimumValidatorCount.fetch(client.endpoint, blockHash)), 1)
  eq(isOkNotNull(await staking.storage.ValidatorCount.fetch(client.endpoint, blockHash)), 105)
  eq(isOkNotNull(await staking.storage.ForceEra.fetch(client.endpoint, blockHash)), "NotForcing")

  const activeEra = new ActiveEraInfo(420, new BN("1756370320000"))
  eq(json(isOkNotNull(await staking.storage.ActiveEra.fetch(client.endpoint, blockHash))), json(activeEra))
  const bondedEra = isOkNotNull(await staking.storage.BondedEras.fetch(client.endpoint, blockHash))
  eq(bondedEra.length, 29)
  eq(bondedEra[0][0], 392)
  eq(bondedEra[0][1], 2352)
  eq(bondedEra[28][0], 420)
  eq(bondedEra[28][1], 2520)
  eq(isOk(await staking.storage.ChillThreshold.fetch(client.endpoint, blockHash)), null)
  eq(isOk(await staking.storage.MinCommission.fetch(client.endpoint, blockHash)), null)

  {
    // Validators
    const accountId = AccountId.from("5Cvfrt7pNqfrpTSMrewUdd7n4W9x9DPhxmmEBcDkS9iSbuD2", true)
    const valiatorsPerf = isOkNotNull(await staking.storage.Validators.fetch(client.endpoint, accountId, blockHash))
    eq(valiatorsPerf.blocked, false)
    eq(valiatorsPerf.commission, 150000000)

    // Validators Iter
    const iter = staking.storage.Validators.iter(client.endpoint, block01Hash)
...

  {
    // Bonded
    const accountId = AccountId.from("5Cvfrt7pNqfrpTSMrewUdd7n4W9x9DPhxmmEBcDkS9iSbuD2", true)
    const bondedAccountId = isOkNotNull(await staking.storage.Bonded.fetch(client.endpoint, accountId, blockHash))
    eq(bondedAccountId.toSS58(), accountId.toSS58())

    // Bonded Iter
    const iter = staking.storage.Bonded.iter(client.endpoint, block01Hash)
    const first = isOkNotNull(await iter.nextKeyValue())
    eq(first[0].toSS58(), "5GuPR92DPMtfRQsTnhNoChi5NXRsYku8Qr5vJK3DdWxhhf1w")
    for (let i = 0; i < 6; ++i) {
      isOkNotNull(await iter.nextKeyValue())
...

  {
    ...
    const claimed = isOkNotNull(await staking.storage.ClaimedRewards.fetch(client.endpoint, 419, accountId, blockHash))
    eq(claimed.length, 1)
    eq(claimed[0], 0)

    // ClaimedRewards Iter
    const iter = isOkNotNull(staking.storage.ClaimedRewards.iter(client.endpoint, 419, blockHash))
    const first = isOkNotNull(await iter.nextKeyValue())
    eq(first[0], 419)
...

  {
    ...
    const claimed = isOkNotNull(await staking.storage.ErasRewardPoints.fetch(client.endpoint, 420, blockHash))
    ...
    const iter = isOkNotNull(staking.storage.ErasRewardPoints.iter(client.endpoint, blockHash))
    const first = isOkNotNull(await iter.nextKeyValue())
    eq(first[0], 407)
...

  {
    ...
    const exposure = isOkNotNull(
      await staking.storage.ErasStakersOverview.fetch(client.endpoint, 420, accountId, blockHash),
    )
    ...
    const iter = isOkNotNull(staking.storage.ErasStakersOverview.iter(client.endpoint, 420, blockHash))
    const first = isOkNotNull(await iter.nextKeyValue())
    eq(first[0], 420)
...

  {
    ...
    const value = isOkNotNull(await staking.storage.ErasStartSessionIndex.fetch(client.endpoint, 420, blockHash))
    eq(value, 2520)

    // ErasStartSessionIndex Iter
    const iter = isOkNotNull(staking.storage.ErasStartSessionIndex.iter(client.endpoint, blockHash))
    const first = isOkNotNull(await iter.nextKeyValue())
    eq(first[0], 407)
    eq(first[1], 2442)
  ...
  {
    // ErasTotalStake
    const value = isOkNotNull(await staking.storage.ErasTotalStake.fetch(client.endpoint, 420, blockHash))
    eq(value.toString(), new BN("4958925200169322168370824260").toString())

    // ErasTotalStake Iter
    const iter = isOkNotNull(staking.storage.ErasTotalStake.iter(client.endpoint, blockHash))
    const first = isOkNotNull(await iter.nextKeyValue())
    eq(first[0], 407)
...

  {
    // ErasValidatorPrefs
    const accountId = AccountId.from("5HSmkdX8oLZWT5ccX9MXGq4ZAnbMWPfgu1ZZAnPkTsfoveAY", true)
    const value = isOkNotNull(
      await staking.storage.ErasValidatorPrefs.fetch(client.endpoint, 420, accountId, blockHash),
    )
    eq(value.commission, 80000000)
    eq(value.blocked, false)

    // ErasValidatorPrefs Iter
    const iter = isOkNotNull(staking.storage.ErasValidatorPrefs.iter(client.endpoint, 420, blockHash))
    const first = isOkNotNull(await iter.nextKeyValue())
    eq(first[0], 420)
    eq(first[1].toSS58(), "5HSmkdX8oLZWT5ccX9MXGq4ZAnbMWPfgu1ZZAnPkTsfoveAY")
  ...
  {
    // ErasValidatorReward
    const value = isOkNotNull(await staking.storage.ErasValidatorReward.fetch(client.endpoint, 419, blockHash))
    eq(value.toString(), new BN("1012783929701037260048293").toString())

    // ErasValidatorReward Iter
    const iter = isOkNotNull(staking.storage.ErasValidatorReward.iter(client.endpoint, blockHash))
    const first = isOkNotNull(await iter.nextKeyValue())
    eq(first[0], 407)
    ...
    // NULL
    const notThere = isOk(await staking.storage.ErasValidatorReward.fetch(client.endpoint, 0, blockHash))
    eq(notThere, null)
  }

  {
    // Ledger
    const accountId = AccountId.from("5HSmkdX8oLZWT5ccX9MXGq4ZAnbMWPfgu1ZZAnPkTsfoveAY", true)
    const value = isOkNotNull(await staking.storage.Ledger.fetch(client.endpoint, accountId, blockHash))
    ...
    const value2 = isOkNotNull(await staking.storage.Ledger.fetch(client.endpoint, accountId2, blockHash))
    ...
    // Ledger Iter
    const iter = isOkNotNull(staking.storage.Ledger.iter(client.endpoint, blockHash))
    const first = isOkNotNull(await iter.nextKeyValue())
...

  {
    ...
    const value = isOkNotNull(await staking.storage.Nominators.fetch(client.endpoint, accountId, blockHash))
    ...
    // Nominators Iter
    const iter = isOkNotNull(staking.storage.Nominators.iter(client.endpoint, blockHash))
    const first = isOkNotNull(await iter.nextKeyValue())
...

  {
    // Payee
    const accountId = AccountId.from("5Higce1mZpyqtgaCwj2QUAL25v8xpE9gQnGUYokNbvN3fiXg", true)
    const value = isOkNotNull(await staking.storage.Payee.fetch(client.endpoint, accountId, blockHash))
    eq(value, "Staked")

    const accountId2 = AccountId.from("5EYCAe5ijiYfAXEth5DCybgrWKqPCuZ4b2E68iqPEMPNdmr2", true)
    const value2 = isOkNotNull(await staking.storage.Payee.fetch(client.endpoint, accountId2, blockHash))
    eq(json(value2), json({ Account: AccountId.from("5EYCAe5ijiYfAXEth5DUidEScpWafTewKhAbgfXDhBG6uTSm") }))

    // Payee Iter
    const iter = isOkNotNull(staking.storage.Payee.iter(client.endpoint, blockHash))
    const first = isOkNotNull(await iter.nextKeyValue())
    eq(first[0].toSS58(), "5Chem9Ssy1cRcoP1jU4D7M5efByHKd1fhBinko3egtbYgXw2")
    eq(first[1], "Staked")
...

  {
    ...
    const value = isOkNotNull(await staking.storage.SlashingSpans.fetch(client.endpoint, accountId, blockHash))
    ...
    // SlashingSpans Iter
    const iter = isOkNotNull(staking.storage.SlashingSpans.iter(client.endpoint, blockHash))
    const first = isOkNotNull(await iter.nextKeyValue())
