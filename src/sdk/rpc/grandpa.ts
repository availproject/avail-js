...
/// Cannot Throw
export async function blockJustification(endpoint: string, at: number): Promise<string | null | ClientError> {
  const res = await call(endpoint, "grandpa_blockJustification", [at])
  if (res instanceof ClientError) return res
  if (res == null) return res
  if (typeof res !== "string") return new ClientError("Justification is not string")
