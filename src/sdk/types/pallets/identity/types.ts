...
export class Data {
  ...
  static decode(decoder: Decoder): DataValue | ClientError {
    ...
    if (variant >= 1 && variant <= 33) {
      ...
      if (bytes instanceof ClientError) return bytes
      return { Raw: bytes }
    }
    if (variant == 34) {
      const blake = decoder.any1(ArrayU8L32)
      if (blake instanceof ClientError) return blake
      return { BlakeTwo256: blake }
    }

    if (variant == 35) {
      const sha = decoder.any1(ArrayU8L32)
      if (sha instanceof ClientError) return sha
      return { Sha256: sha }
    }

    if (variant == 36) {
      const keccak = decoder.any1(ArrayU8L32)
      if (keccak instanceof ClientError) return keccak
      return { Keccak256: keccak }
    }

    if (variant == 37) {
      const shaThree = decoder.any1(ArrayU8L32)
      if (shaThree instanceof ClientError) return shaThree
      return { ShaThree256: shaThree }
    }

    return new ClientError("Unknown Data")
  }
...
export class DoubleData {
  ...
  static decode(decoder: Decoder): [DataValue, DataValue] | ClientError {
...
export class IdentityInfo {
  ...
  static decode(decoder: Decoder): IdentityInfo | ClientError {
    const additional = decoder.vec(DoubleData)
    if (additional instanceof ClientError) return additional

    const res1 = decoder.any5(Data, Data, Data, Data, Data)
    if (res1 instanceof ClientError) return res1

    const pgpFingerprint = decoder.option(ArrayU8L20)
    if (pgpFingerprint instanceof ClientError) return pgpFingerprint

    const res2 = decoder.any2(Data, Data)
    if (res2 instanceof ClientError) return res2
