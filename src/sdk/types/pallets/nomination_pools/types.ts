...
export class ClaimPermission {
  ...
  static decode(decoder: Decoder): ClaimPermissionValue | ClientError {
    const variant = decoder.byte()
    if (variant instanceof ClientError) return variant
    ...
    return new ClientError("Cannot decode ClaimPermission")
  }
...
export class PoolState {
  ...
  static decode(decoder: Decoder): PoolStateValue | ClientError {
    const variant = decoder.byte()
    if (variant instanceof ClientError) return variant
    ...
    return new ClientError("Cannot decode PoolState")
  }
...
export class ConfigOpAccountId {
  ...
  static decode(decoder: Decoder): ConfigOpAccountIdValue | ClientError {
    const variant = decoder.byte()
    if (variant instanceof ClientError) return variant
    ...
    return new ClientError("Cannot decode ConfigOpAccountId")
  }
