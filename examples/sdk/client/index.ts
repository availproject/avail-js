...
async function blockHeaderExample(client: Client): Promise<null | AvailError> {
  ...
  // Custom Block Header
  {
    ...
    if (blockHeader instanceof AvailError) return blockHeader
...
async function blockStateExample(client: Client): Promise<null | AvailError> {
  // Custom Block State (unknown block hash)
  {
    const loc = { hash: H256.default(), height: 100 }
    const blockState = await client.blockState(loc)
    if (blockState instanceof AvailError) return blockState
  ...
  // Best Block State
  {
    ...
    if (loc instanceof AvailError) return loc
...
async function blockHeightExample(client: Client): Promise<null | AvailError> {
  const blockHash = "0x149d4a65196867e6693c5bc731a430ebb4566a873f278d712c8e6d36aec7cb78"

  // Block Hash to Block Height
  {
    const blockHeight = (await client.blockHeight(blockHash))!
    if (blockHeight instanceof AvailError) return blockHeight
...
async function blockHashExample(client: Client): Promise<null | AvailError> {
